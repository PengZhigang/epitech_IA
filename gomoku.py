...
SCORE_FIVE, SCORE_FOUR, SCORE_SFOUR = 100000, 10000, 100
...
class Gomoku:
    ...
    def __init__(self, len: int):
        self.len = len
        self.board = [[0 for x in range(len)] for y in range(len)]
        for i in range(len):
            for j in range(len):
                self.board[i][j] = 0
        self.record = [[[0, 0, 0, 0] for x in range(len)] for y in range(len)]
        self.count = [[0 for x in range(CHESS_TYPE_NUM)] for i in range(2)]
        self.pos_score = [[(7 - max(abs(x - 7), abs(y - 7))) for x in range(len)] for y in range(len)]
        hash_key = hash(str(self.board))
        self.zobrist[hash_key] = "0"

    def board_is_empty(self, y, x):
        if (self.board[x][y] == 0):
            return True
        return False

    def add_board(self, y, x, player):
        len = self.len
        if (x < len and y < len and Gomoku.board_is_empty(self, y, x)):
    ...
    def genmove(self, board, turn):
        ...
        # FIXME: decrease think time: only
        # consider limited moves with higher scores
    ...
    def __search(self, board, turn, depth, alpha=SCORE_MIN, beta=SCORE_MAX):
        if(hash(str(board)) in self.zobrist.keys()):
            score = int(self.zobrist[hash(str(board))])
        else:
            score = self.evaluate(board, turn)
            self.zobrist[hash(str(board))] = score
